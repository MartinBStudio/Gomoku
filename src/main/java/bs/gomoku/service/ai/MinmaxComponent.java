...
public class MinmaxComponent {
    ...
    // Constructor
    public MinmaxComponent(BoardComponent boardComponent) {
        this.boardComponent = boardComponent;
    }

    // Getter function for the winScore
    private static int getWinScore() {
        return winScore;
    }
    ...
    private static double evaluateBoardForWhite(BoardComponent boardComponent, boolean blacksTurn) {
        evaluationCount++;

        // Get board score of both players.
        double blackScore = getScore(boardComponent, true, blacksTurn);
        double whiteScore = getScore(boardComponent, false, blacksTurn);
    ...
    private static int getScore(BoardComponent boardComponent, boolean forBlack, boolean blacksTurn) {

        // Read the board
        int[][] boardMatrix = boardComponent.getBoardMatrix();
     ...
     * */
    private static Object[] minimaxSearchAB(
            int depth, BoardComponent boardComponent, boolean max, double alpha, double beta) {

        // Last depth (terminal node), evaluate the current board score.
        if (depth == 0) {
            Object[] x = {evaluateBoardForWhite(boardComponent, !max), null, null};
            return x;
        }
         ...
         *                  (Move N)
         */
        ArrayList<int[]> allPossibleMoves = boardComponent.generateMoves();
        ...
        if (max) {
            ...
            for (int[] move : allPossibleMoves) {
                ...
                BoardComponent dummyBoardComponent = new BoardComponent(boardComponent);

                // Play the move on that temporary board without drawing anything
                dummyBoardComponent.addStoneNoGUI(move[1], move[0], false);
                ...
                // lower depth.
                Object[] tempMove = minimaxSearchAB(depth - 1, dummyBoardComponent, !max, alpha, beta);
            ...
            for (int[] move : allPossibleMoves) {
                ...
                // Play the move on that temporary board without drawing anything
                dummyBoardComponent.addStoneNoGUI(move[1], move[0], true);
    ...
    private static Object[] searchWinningMove(BoardComponent boardComponent) {
        ArrayList<int[]> allPossibleMoves = boardComponent.generateMoves();
        ...
        for (int[] move : allPossibleMoves) {
            ...
            if (getScore(dummyBoardComponent, false, false) >= winScore) {
    ...
    private static int evaluateHorizontal(
            int[][] boardMatrix, boolean forBlack, boolean playersTurn) {

        int consecutive = 0;
        // blocks variable is used to check if a consecutive stone set is blocked by the opponent or
        // the board border. If the both sides of a consecutive set is blocked, blocks variable will be
        // 2
        // If only a single side is blocked, blocks variable will be 1, and if both sides of the
        // consecutive
    ...
    // blocks: Number of blocked sides of the set (2: both sides blocked, 1: single side blocked, 0:
    // both sides free)
        ...
        switch (count) {
            ...
            case 4: {
                ...
                if (currentTurn) return winGuarantee;
                    ...
                    if (blocks == 0) return winGuarantee / 4;
                        ...
                        // (Opponent can only place a stone that will block the remaining side, otherwise the
                        // game is lost
            ...
            case 3: {
                ...
                if (blocks == 0) {
                    ...
                    // (User places another stone to make the set 4 consecutive, opponent can only block one
                    // side)
                    // However the opponent may win the game in the next turn therefore this score is lower
                    // than win
                    // guaranteed scores but still a very high score.
                    if (currentTurn) return 50000;
                        // If it's the opponent's turn, this set forces opponent to block one of the sides of
                        // the set.
    ...
    protected int[] calculateNextMove(int depth) {
        ...
        Object[] bestMove = searchWinningMove(boardComponent);
        ...
        if (bestMove != null) {
            ...
            bestMove = minimaxSearchAB(depth, boardComponent, true, -1.0, getWinScore());
            if (bestMove[1] == null) {
                move = null;
            } else {
                move[0] = (Integer) (bestMove[1]);
